...
type OutputStats struct {
	lines int64
	bytes int64
	// WARNING: only add fields after this line to preserve the 64-bit alignment (for sync/atomic)
}
...
func init() {
	...
	flag.BoolVar(&logging.nolog, "nolog", false, "turn off all logging and don't create any files")
...
func (l *loggingT) header(s severity) *buffer {
	...
	buf := l.getBuffer()
	if logging.prefix != "" {
		buf.WriteString(logging.prefix)
		buf.WriteByte(' ')
	}
...
func (sb *syncBuffer) rotateFile(now time.Time) error {
	if sb.logger != nil && sb.logger.nolog {
		return nil
	}
	if sb.file != nil {
		sb.Flush()
...
//XXX These Exit functions are badly designed. It should take exit code. Not exit with code == 1 all the time.
...
// Finishf takes exit code, logs msg, and uses the proper code for exit()
func Finishf(code int, format string, args ...interface{}) {
	// XXX cannot call Infof() directly due to use of runtime.Caller(3) in header() and output()
	logging.printf(infoLog, format, args...)
	logging.printf(infoLog, "Exiting with code %d", code)
	timeoutFlush(10 * time.Second)
	os.Exit(code)
}

// ErrorLogf allows error to return to caller  after logging
func ErrorLogf(format string, args ...interface{}) error {
